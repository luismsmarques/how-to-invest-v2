	...
	public static function boot_data(): array {
		...
		return array(
			...
			'i18n'        => array(
				...
				'webm_note'    => $pt ? 'Em Chrome/Safari recentes grava diretamente em MP4. Se o browser não suportar, sai em WebM.' : 'Recent Chrome/Safari record straight to MP4. If unsupported, output is WebM.',
				...
				'mp4_note'     => $pt ? 'Tenta gravar MP4 nativamente (rápido). Só se o browser não suportar é que usa o conversor ffmpeg (~30 MB na 1.ª vez).' : 'Records MP4 natively when possible (fast). Only falls back to the ffmpeg converter (~30 MB first time) if unsupported.',
